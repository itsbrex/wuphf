...
//
// App building and wiki maintenance used to be dedicated default agents
// (App Builder, Librarian). Both are retired: the capabilities are skills
// every agent carries. The founder's model, verbatim: "app building is a
// default system skill. it is enabled by default for every agent, can be
...
//   - For a system skill OwnerAgents is ignored. The effective assignment
//     is the whole roster minus DisabledAgents, so every agent — including
//     one hired five minutes from now — carries the skill until a human
//     switches it off for that agent.
//   - enable-for / disable-for flip DisabledAgents; archive, reject,
	...
	return []systemSkillSpec{
		{
			...
			title:       "App building",
			description: "Build and update internal tools (Apps) for the office. A system skill every agent carries.",
			content: "Every agent can build Apps — small internal tools published under the Apps rail.\n\n" +
				...
				"Builds flow through the host-owned build and publish gates regardless of which agent registers them.",
		},
		{
			name:        systemSkillWikiMaintenance,
			title:       "Wiki maintenance",
			description: "Keep the team wiki current: draft in notebooks, promote for review, write directly only on the human's explicit ask. A system skill every agent carries.",
			content: "Every agent maintains the team wiki.\n\n" +
...
// the whole roster minus DisabledAgents. Caller must hold b.mu.
func (b *Broker) systemSkillEffectiveOwnersLocked(sk *teamSkill) []string {
	disabled := make(map[string]struct{}, len(sk.DisabledAgents))
...
// for the agent. A missing skill record reads as enabled — the gate exists
// to honor an explicit per-agent switch-off, and a broken skills read must
// never brick every agent's core capabilities.
func (b *Broker) SystemSkillEnabledFor(skillName, agent string) bool {
	agent = normalizeActorSlug(agent)
	if agent == "" {
		return true
	}
	b.mu.Lock()
	...
	for i := range b.skills {
		...
		for _, off := range sk.DisabledAgents {
			if normalizeActorSlug(off) == agent {
				return false
			}
		}
		return true
...
func guardSystemSkillMutation(sk *teamSkill, verb string) string {
	...
	case "archive", "reject", "delete", "disable":
		return fmt.Sprintf("%s is a system skill: it cannot be %sd. Disable it per agent with /skills/{name}/disable-for instead.", sk.Name, verb)
	}
	return ""
}
