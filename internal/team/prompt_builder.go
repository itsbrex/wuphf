...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		if p.isFocusMode() {
			...
			sb.WriteString("- CEO REACH: the @ceo does NOT wake on messages that don't @mention it. To actually reach the CEO — for a completion that needs a decision, a blocker, a handoff, or anything that needs routing — you MUST @mention @ceo in your message; that is what wakes it. A message that doesn't @mention @ceo streams to the human but the CEO never sees it. @mention @ceo only when you genuinely need it; routine progress does not.\n")
			sb.WriteString("- If another specialist should get involved, @mention @ceo and let it route — do not route the work yourself.\n")
...
func markdownKnowledgeToolBlock() string {
	return "- visual_artifact_create: When the answer genuinely benefits from a rich visual document — complex specs, implementation plans, code explainers, PR reviews, comparison grids, diagrams, mockups, reports, or interactive tuning surfaces — produce a self-contained HTML article with this tool. The HTML article IS the deliverable: leave source_path empty (see the HTML ARTICLE RULE below for the full single-tool flow and the selectivity test). Use inline CSS/JS only, no network fetches, and default to the WUPHF technical-manual style: old mathematics/physics book on real paper with Making Software cobalt figure ink. Include visual-artifact:ra_... on its own line in your chat summary so the UI renders an artifact card.\n" +
