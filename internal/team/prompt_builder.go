...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		if p.isFocusMode() {
			...
			sb.WriteString("- Chief of Staff REACH: the @ceo does NOT wake on messages that don't @mention it. To actually reach the Chief of Staff — for a completion that needs a decision, a blocker, a handoff, or anything that needs routing — you MUST @mention @ceo in your message; that is what wakes it. A message that doesn't @mention @ceo streams to the human but the Chief of Staff never sees it. @mention @ceo only when you genuinely need it; routine progress does not.\n")
