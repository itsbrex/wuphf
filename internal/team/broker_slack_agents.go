...
// FOREIGN registrations (Kind == slack) and SPAWNED office agents
		...
		if m.Slug == slug {
			if boundID == userID {
				if m.Provider.Kind != provider.KindSlack {
					// A SPAWNED agent already owns this slug + user id.
					// Re-registering it as FOREIGN would flip its runtime to
					// the gateway kind and stop its turns from dispatching.
					return fmt.Errorf("slug %q is a spawned office agent, not a foreign one", slug)
				}
