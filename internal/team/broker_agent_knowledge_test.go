...
func newKnowledgeBroker(t *testing.T) (*Broker, *Repo, func()) {
	...
	b.mu.Lock()
	b.wikiWorker = worker
	// The asking agent has to be on the roster: the approval card is raised in
	// that agent's home channel, and a non-member has none.
...
// writeNote drops a notebook note straight onto disk. Agents write these
// through the notebook path; for these tests the bytes are what matter.
func writeNote(t *testing.T, repo *Repo, agent, file, body string) string {
	t.Helper()
	rel := agentNotebookRel(agent, file)
...
func TestAgentKnowledgeIsScopedToOneAgent(t *testing.T) {
	...
	dwight := b.agentKnowledgePages("dwight")
	if len(dwight) != 2 {
		t.Fatalf("dwight pages = %d, want 2: %+v", len(dwight), dwight)
	}
	for _, p := range dwight {
		if p.Agent != "dwight" {
			t.Fatalf("page %q attributed to %q, want dwight", p.Title, p.Agent)
		}
	...
	// An agent that has written nothing knows nothing, and says so. No
	// placeholder page, no borrowed page from a teammate.
	if pages := b.agentKnowledgePages("creed"); len(pages) != 0 {
		t.Fatalf("creed pages = %+v, want an honest empty set", pages)
	}
	if pages := b.agentKnowledgePages("kevin"); len(pages) != 1 || pages[0].Agent != "kevin" {
...
// can only ever address the named agent's own notebook.
func TestKnowledgePromotionRefusesForeignPaths(t *testing.T) {
	...
	cases := []struct {
		...
		{"another agent's notebook", "dwight", "agents/kevin/notebook/chili.md"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := b.snapshotKnowledgePromotion(tc.agent, tc.path, "", ""); err == nil {
...
// sanitizer: agent prose cannot open a new line and pose as a broker-authored
// section of the card.
...
func TestHumanPromotionIsBoundToTheBytesTheHumanRead(t *testing.T) {
	...
	pages := b.agentKnowledgePages("dwight")
	if len(pages) != 1 {
		t.Fatalf("pages = %+v", pages)
	}
	shaTheHumanSaw := pages[0].Promotion.ContentSHA

	// The agent rewrites the note after the human read it.
...
//
// An agent asks a human to promote a page. The human sees that page. Before
// they answer, the agent rewrites the note on disk. The bytes that land in the
...
func TestApprovedPromotionWritesTheApprovedBytesNotTheCurrentFile(t *testing.T) {
	...
	// The agent rewrites the note between the ask and the answer.
	...
	// Provenance survives promotion: which agent knew it, where it came from,
...
func TestPromotionApprovalIsPerPage(t *testing.T) {
	...
	// The same page asked twice collapses, so a retrying agent cannot spam the
