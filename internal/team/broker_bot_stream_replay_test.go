...
// TestHandleBotStreamEmitsReplayEndBoundary pins the contract that
// handleBotStream sends a named `event: replay-end` SSE entry between
...
//
// We pre-seed the bot's stream buffer with a fake history line, hit
...
func TestHandleBotStreamEmitsReplayEndBoundary(t *testing.T) {
	...
	srv := httptest.NewServer(b.requireAuth(b.handleBotStream))
	defer srv.Close()
