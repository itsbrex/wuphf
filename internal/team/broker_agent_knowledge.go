...
// broker_agent_knowledge.go — Knowledge is what an AGENT knows.
//
// Knowledge used to hang off an app. It does not belong there: a page is
// something a particular agent learned, so it belongs to that agent. The
// storage for that already existed and is not new here — the wiki git repo has
// carried per-agent draft notes at agents/{slug}/notebook/*.md since the
...
//
//	agents/{slug}/notebook/*.md   what ONE agent knows (private to that agent)
...
//     POST /agent-knowledge/promote
//  2. An agent judges a page worth sharing and ASKS. The broker raises an
//     approval card; the promotion happens only if a human approves it.
//     POST /agent-knowledge/promotion-request
//
...
//     it promotes the SNAPSHOT, never a re-read of the file. An agent that
//     rewrites its note between asking and being approved changes nothing about
//     what lands in the wiki.
//   - AGENT TEXT IS NEVER TRUSTED. The agent supplies only a source path and a
//     rationale. The rationale and the note title are pushed through
//     sanitizeKnowledgeCardText before they reach the card, so agent prose
...
//     deliberately NOT a field of the POST /requests body, so an agent cannot
...
// and gains a broker-authored "Provenance" section naming the agent, the source
// path, the approver, the date, and the content hash. That footer is written by
// the broker, is not agent-controllable, and is the citation for the page.

const (
	// agentKnowledgeMaxPages bounds one agent's page list.
	agentKnowledgeMaxPages = 200
	// agentKnowledgeMaxNoteBytes bounds a single promotable note. A note larger
	// than this is readable in the agent's Knowledge but cannot be promoted:
	// nobody reviews a megabyte on an approval card, and an approval nobody read
	// is not an approval.
	agentKnowledgeMaxNoteBytes = 256 * 1024
...
// knowledgePromotionSpec is the immutable snapshot of one promotion an agent
// asked a human to approve. It is created only by requestKnowledgePromotion,
// which reads Content off disk itself; nothing an agent sends populates it.
type knowledgePromotionSpec struct {
	// Agent is the slug whose notebook the page came from.
	Agent string `json:"agent"`
	// SourcePath is the repo-relative note (agents/{slug}/notebook/{file}.md).
	...
	// Reason is the agent's sanitized rationale for asking.
...
// private to the agent, waiting on a human, or already in the shared wiki.
...
// ── Reading one agent's knowledge ────────────────────────────────────────────

// knowledgeWikiRoot resolves the wiki repo root. It prefers the live worker's
// root and falls back to the configured path, so an agent's knowledge is
...
// agentNotebookRel builds the repo-relative path of one notebook note.
func agentNotebookRel(agent, file string) string {
	return "agents/" + agent + "/notebook/" + file
}

// loadAgentNotebookPages reads one agent's notebook notes into knowledge pages,
// newest-titled-first by title. Each page carries its Agent and SourcePath so
// the reader can cite where the knowledge came from and the promote path can
// address it. A missing notebook yields nil — an agent that has not written
// anything down knows nothing, and says so.
//
// Extracted from loadLegacyKnowledgePages (which walks every agent) so the
// per-agent surface and the preservation pass share one parser; the "Notebook ·
// <agent>" category and id scheme are unchanged.
func loadAgentNotebookPages(root, agent string) []appKnowledgePage {
	if validateNotebookSlug(agent) != nil {
		return nil
	}
	notes, err := os.ReadDir(filepath.Join(root, "agents", agent, "notebook"))
	if err != nil {
		return nil
	}
	...
	for _, note := range notes {
		...
		id := "notebook-" + slugifyKnowledgeID(agent+"-"+note.Name())
		page, ok := legacyPageFromFile(path, id, "Notebook · "+agent)
		if !ok {
			continue
		}
		page.Agent = agent
		page.SourcePath = agentNotebookRel(agent, note.Name())
		pages = append(pages, page)
	}
	sort.Slice(pages, func(i, j int) bool { return pages[i].Title < pages[j].Title })
	if len(pages) > agentKnowledgeMaxPages {
		fmt.Fprintf(os.Stderr, "broker: agent %s knowledge capped at %d of %d pages\n",
			agent, agentKnowledgeMaxPages, len(pages))
		pages = pages[:agentKnowledgeMaxPages]
	}
	return pages
}

// agentKnowledgePages loads one agent's pages and stamps each with its live
// promotion standing: already in the wiki, waiting on a human, or private.
func (b *Broker) agentKnowledgePages(agent string) []appKnowledgePage {
	root := b.knowledgeWikiRoot()
	pages := loadAgentNotebookPages(root, agent)
	pending := b.pendingPromotionsBySource(agent)
	for i := range pages {
...
// agent, so the reader can show "waiting on you" instead of offering a second
// promote button for a page already on the human's desk.
func (b *Broker) pendingPromotionsBySource(agent string) map[string]string {
	out := map[string]string{}
	b.mu.Lock()
	defer b.mu.Unlock()
	for i := range b.requests {
		spec := b.requests[i].KnowledgePromotion
		if spec == nil || spec.Agent != agent || spec.PromotedPath != "" {
			continue
		}
...
// ── Sanitizing agent-authored text for the approval card ─────────────────────

// sanitizeKnowledgeCardText collapses control characters and structural
// delimiters in agent-authored text into safe inline text. It is the package
...
// forged label at line start lets agent prose pose as a broker-authored section,
...
// resolveAgentNotePath validates that relPath is exactly one of `agent`'s own
// notebook notes and returns its absolute path. It refuses traversal, absolute
// paths, non-canonical spellings, another agent's notebook, and anything outside
// the notebook subtree — so a promotion can only ever address the asking agent's
// own knowledge.
...
//
// The footer is deliberately outside the approved bytes and outside agent
// control. It is what keeps the honesty rule true after promotion — the page
// still says which agent knew it, where it came from, who approved it, and
...
func renderPromotedArticle(spec knowledgePromotionSpec, approvedBy, promotedAt string) string {
	...
	fmt.Fprintf(&b, "Promoted from @%s's knowledge (`%s`) on %s.\n",
		spec.Agent, spec.SourcePath, promotedAt)
...
func (b *Broker) snapshotKnowledgePromotion(agent, sourcePath, targetPath, reason string) (knowledgePromotionSpec, error) {
	root := b.knowledgeWikiRoot()
	abs, err := resolveAgentNotePath(root, agent, sourcePath)
	if err != nil {
	...
	if info.IsDir() || info.Size() > agentKnowledgeMaxNoteBytes {
		return knowledgePromotionSpec{}, fmt.Errorf("knowledge: %q is not a promotable note (max %d bytes)", sourcePath, agentKnowledgeMaxNoteBytes)
	}
...
// one page. The agent never promotes; it asks, and the answer decides.
//
...
func (b *Broker) requestKnowledgePromotion(agent, sourcePath, targetPath, reason string) (string, error) {
	if worker := b.WikiWorker(); worker == nil || worker.Repo() == nil {
	...
	spec, err := b.snapshotKnowledgePromotion(agent, sourcePath, targetPath, reason)
	if err != nil {
		return "", err
	}
	dedupeKey := knowledgePromotionDedupePrefix + spec.ContentSHA

	question := fmt.Sprintf("@%s wants to promote %q into the team wiki. Approve?", agent, spec.Title)
	...
	ctxb.WriteString("Approving promotes exactly the text shown on this card, plus a provenance footer naming " +
		agent + " as its author. Editing the note after this card is raised does not change what lands.")
	...
	channel, err := b.homeChannelForLocked(agent)
	if err != nil {
	...
	req := humanInterview{
		...
		// office. The agent keeps working; the page waits.
	...
	b.appendActionLocked("request_created", "office", channel, agent,
...
func (b *Broker) maybePromoteKnowledgeFromApproval(requestID, actor string) {
	...
	msg := channelMessage{
		...
		From:      "system",
		Channel:   channel,
		Tagged:    uniqueSlugs([]string{spec.Agent}),
		Timestamp: time.Now().UTC().Format(time.RFC3339),
	}
	if err != nil {
	...
	msg.Content = fmt.Sprintf("Promoted %q from @%s's knowledge into the team wiki at %s (%s).",
		spec.Title, spec.Agent, spec.TargetPath, sha)
	b.appendMessageLocked(msg)
	_ = b.saveLocked()
}
...
// handleAgentKnowledge serves GET /agent-knowledge?agent=<slug> — the pages one
// agent knows, each with its promotion standing. An agent with no notes returns
// an empty list; the reader shows that honestly rather than inventing pages.
func (b *Broker) handleAgentKnowledge(w http.ResponseWriter, r *http.Request) {
	...
	if err := validateNotebookSlug(agent); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "a valid agent slug is required"})
		return
	}
	pages := b.agentKnowledgePages(agent)
	if pages == nil {
...
// handleAgentKnowledgePromote serves POST /agent-knowledge/promote — a HUMAN
...
// read; if the agent rewrote the note in between, this answers 409 and the human
// re-reads rather than silently publishing something else.
func (b *Broker) handleAgentKnowledgePromote(w http.ResponseWriter, r *http.Request) {
	...
	spec, err := b.snapshotKnowledgePromotion(strings.TrimSpace(body.Agent), body.SourcePath, body.TargetPath, "")
	if err != nil {
...
// handleAgentKnowledgePromotionRequest serves POST
// /agent-knowledge/promotion-request — an AGENT judging a page worth sharing and
// asking a human. It never writes to the wiki; it raises a card.
func (b *Broker) handleAgentKnowledgePromotionRequest(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	id, err := b.requestKnowledgePromotion(agent, body.SourcePath, body.TargetPath, body.Reason)
	if err != nil {
		writeKnowledgePromotionError(w, err)
		return
	}
