...
// agent_message — one agent sends one direct message to one peer.
//
...
// is to make agent-to-agent conversation possible at all, so the consult relay
...
// agent cannot mint a conversation with an invented peer. The notifier then
// delivers to the DM partner by the same path as any other DM.
//
// No prompt anywhere instructs an agent to use this. That is intentional: the
// mechanism should be observable and safe before anything is told to reach for
// it.

// AgentMessageArgs is the tool's input.
type AgentMessageArgs struct {
	To      string `json:"to" jsonschema:"Slug of the teammate to message directly. Must be an existing agent from the roster."`
...
func handleAgentMessage(ctx context.Context, _ *mcp.CallToolRequest, args AgentMessageArgs) (*mcp.CallToolResult, any, error) {
	...
	// The recipient must exist. Checked against the roster the agent can
	// already see (the AVAILABLE AGENTS directory is public), so this refuses
	// invented peers without hiding who is real.
	known, err := agentRosterSlugs(ctx)
	if err != nil {
		return toolError(err), nil, nil
	}
	if _, ok := known[to]; !ok {
	...
	// The pair DM slug is deterministic and order-independent, so both agents
...
// agentRosterSlugs returns the set of real agent slugs, lowercased.
func agentRosterSlugs(ctx context.Context) (map[string]struct{}, error) {
