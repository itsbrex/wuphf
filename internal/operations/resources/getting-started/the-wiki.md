...
Everyone. Humans read the wiki when they want to understand the team. Agents read it as first-class consumers every time they pick up work, which is what makes them start each issue already knowing your company instead of asking you to recap it.

A page that no one reads, human or agent, is a page going stale. A page that gets read a lot is doing its job. The team tracks both, so the brain stays sharp instead of bloating with notes nobody needs.
