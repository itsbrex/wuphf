...
WUPHF is not a CRM, and it does not ask you to migrate into one. Your agents operate on the CRM you already have. Think of the team as the team that keeps the one you own honest, not another system to maintain.

Start with a single hygiene pass, for example "find every duplicate account and every deal missing an owner, then propose a cleanup." Review the plan, approve it, and let the revops agent run while the analyst checks the funnel for anything that looks off.

## Connect a GitHub repository

The fastest way to give your team something real to do is to connect a GitHub repository. Once a repo is connected, your agents can read the code, understand the project, and pick up work against it the same way a new engineer would after their first week.
...
A connected repo turns vague requests into concrete work. Instead of describing a bug from memory, you point your agent at the file, the function, and the failing case, and it goes from there.
