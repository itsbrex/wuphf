...
// `gawkbot workspace shred <name>` — back up and wipe a workspace.
//
...
//     today's `gawkbot shred` confirm flow.
...
func runWorkspaceShred(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "gawkbot workspace shred — burn a workspace down")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  gawkbot workspace shred <name>             Back up wiki/skills/chats/context (30-day retention)")
		fmt.Fprintln(os.Stderr, "  gawkbot workspace shred <name> --permanent  Skip backup (irreversible)")
		fmt.Fprintln(os.Stderr, "  gawkbot workspace shred <name> --yes        Skip the confirm prompt (scripts only)")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Restorable for 30 days unless --permanent. List shredded workspaces with")
		fmt.Fprintln(os.Stderr, "`gawkbot workspace list --trash` and bring one back with `gawkbot workspace restore`.")
	}
	...
	_, _ = fmt.Fprintf(os.Stdout, "Shredded %q. Wiki, skills, chats, and context backed up; restorable for 30 days via `gawkbot workspace restore`.\n", name)
}
...
func shredConfirmFromReader(in io.Reader, out io.Writer, name string, permanent bool) (bool, error) {
	if name == "main" {
		...
		_, _ = fmt.Fprintln(out, "         This is the workspace gawkbot created from your pre-multi-workspace install.")
