...
vi.mock("../api/client", () => ({
  ...
  // the botStreamURL caller has to merge query strings safely.
...
describe("appendStreamLine", () => {
  ...
  it("encodes slug and task to keep odd characters from breaking the URL", () => {
    expect(botStreamURL("a/b", "t&1")).toBe(
...
describe("useBotStream phase + idle behavior", () => {
  beforeEach(() => {
  ...
  it("ignores non-headless_event JSON entries with a status field", () => {
    // The bot stream carries multiple event shapes today (raw provider
