 ...
 * agent, so "which channel does this belong in?" is now always answered by
 * naming an AGENT. That makes this the canonical answer, and the reason there
...
/**
 * The DM channel an agent's work belongs in, or "" when no agent is named.
 *
 ...
 * `directChannelSlug("")` cheerfully returns `"human__"`, a slug for an agent
 ...
 */
export function agentHomeChannel(agentSlug: string | undefined | null): string {
