 ...
 * `setInterval` running for the entire bot rail — not one per pill —
...
// outside an `BotEventTickProvider` (mostly tests). The provider replaces
 ...
 * ticking after BotList unmounts (dev hot-reload, route changes,
 * multi-tab), which the eng review flagged as a CRITICAL test gap.
 */
export function BotEventTickProvider({ children }: TickProviderProps) {
...
interface BotEventPillProps {
  slug: string;
  /**
   * Domain bot role string ("engineer", "designer", etc.) used as a
   * lookup key into `pickIdleCopy`. Named `botRole` rather than `role`
...
/**
 * Inline event pill rendered in place of the legacy `.sidebar-bot-task`
 * line. Subscribes to the per-slug snapshot in Zustand and the shared 1Hz
 * tick from `BotEventTickProvider`, derives pill state via
 ...
 */
export function BotEventPill({
  slug,
  agentRole,
  fallbackTask,
}: BotEventPillProps) {
  const snapshot = useAppStore((s) => s.botActivitySnapshots[slug]);
