/**
 * TeachWorkflowModal — show one agent a workflow on a screenshare, once.
 *
 ...
 *   4. "Send to @agent" formats it (teachWorkflowSeed) and posts it into the
 *      agent's own DM channel — the same channel the Chat tab writes to, so the
 *      agent is woken and answers there. This component never plans, drafts, or
 *      summarizes the workflow itself.
 *
 * The capture is app-agnostic at the broker (POST /observe/browser takes no
 * app or agent), so pointing it at an agent needed no server change: the only
 * agent-scoped part is where the result is delivered.
 *
...
import { buildAgentWorkflowSeed } from "./teachWorkflowSeed";
...
export function TeachWorkflowModal({
  ...
  async function send() {
    ...
    try {
      await postMessage(
        buildAgentWorkflowSeed({ agentSlug, goal, screens }),
...
/**
 * The workflow reached the agent. Say only that, and say what is still open:
 * whether the agent can run every step is the agent's answer to give, not this
 * modal's to claim.
 */
function Sent({
  who,
...
function TeachError({
  ...
  return (
    <div className="teach-workflow-result" role="alert">
      <div className="teach-workflow-result-title">
        {sendFailed ? "This did not reach the agent" : "The reading stopped"}
      </div>
      <p className="teach-workflow-note">
        {sendFailed
          ? "The screens that were read are still here, but the message did not send, so the agent has not seen any of it."
