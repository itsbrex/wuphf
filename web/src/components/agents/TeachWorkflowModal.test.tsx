...
// the whole screenshare -> agent-DM chain rather than a stubbed middle.
...
describe("TeachWorkflowModal", () => {
  ...
  it("sends the captured workflow into the agent's own DM channel", async () => {
    const stream = mockObserveStream();
    const view = renderModal();
    view.start("File the weekly expense report");
    await waitFor(() => expect(postStream).toHaveBeenCalled());
  ...
  it("says the agent has not seen anything when the send fails", async () => {
    const stream = mockObserveStream();
    ...
    await waitFor(() =>
      expect(
        view.getByText("This did not reach the agent"),
      ).toBeInTheDocument(),
    );
