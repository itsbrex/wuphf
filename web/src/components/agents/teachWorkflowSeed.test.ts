...
describe("buildAgentWorkflowSeed", () => {
  it("keeps the screens in the order they were used", () => {
    const seed = buildAgentWorkflowSeed({
      agentSlug: "planner",
  ...
  it("addresses the agent it is teaching", () => {
    const seed = buildAgentWorkflowSeed({
      agentSlug: "planner",
      goal: "File the weekly expense report",
      screens,
  ...
  // The honesty gate: the agent's first reply has to name what it cannot run,
  // so nothing downstream can present an untried workflow as a working one.
  it("asks the agent to name the steps it cannot do yet", () => {
    const seed = buildAgentWorkflowSeed({
      agentSlug: "planner",
      goal: "File the weekly expense report",
      screens,
