...
/**
 * Knowledge, scoped to one agent, with the promotion gate into the shared team
 ...
 */
const meta: Meta<typeof AgentKnowledgePanel> = {
  title: "Features/Knowledge/AgentKnowledgePanel",
  component: AgentKnowledgePanel,
};

export default meta;
type Story = StoryObj<typeof AgentKnowledgePanel>;
...
function seeded(result: AgentKnowledgeResult, pending?: PendingPromotion) {
  ...
  return function Seeded() {
    return (
      <QueryClientProvider client={qc}>
        <AgentKnowledgePanel agentSlug={result.agent} />
      </QueryClientProvider>
    );
  };
}

/** A page only this agent uses, with the promotion offer. */
...
/**
 * The agent asked; the decision is with the human. The card shows the exact
...
/** An agent that has written nothing knows nothing, and says so. */
...
export const WikiUnavailable: Story = {
  render: seeded({ agent: AGENT, wikiWritable: false, pages: [page()] }),
};
