/**
 * AgentKnowledgePanel — Knowledge, scoped to one agent.
 *
 * Knowledge is what THIS agent knows: the pages in its own notebook. It is
 * private to the agent until a human promotes it into the team wiki, which is
 * the shared, trusted surface everyone reads.
 *
 * Two ways a page gets promoted, one gate:
 *  - A human reads a page here and promotes it.
 *  - The agent judges a page worth sharing and asks; the request lands in the
 *    decision inbox as an approval card, and nothing moves until it is answered.
 *
 * Nothing on this surface is invented. An agent that has written nothing shows
 * an empty state, not a placeholder page, and every page names the note it came
 * from so a reader can go check.
 *
 * Rendering note: page text is rendered as TEXT. Agent-authored prose never
...
import "../../styles/agent-knowledge.css";
...
function PromotionPill({ state }: { state: KnowledgePromotionState }) {
  ...
  return (
    <span className={`agent-knowledge-pill is-${state}`}>
...
/**
 * The review for a promotion an agent asked for.
 *
 * This renders the SNAPSHOT the broker took when the agent asked — the exact
 ...
 *
 * The snapshot is shown verbatim in a <pre>, as text. Agent prose is untrusted
...
export function AgentKnowledgePanel({ agentSlug }: AgentKnowledgePanelProps) {
  const query = useQuery({
    queryKey: ["agent-knowledge", agentSlug],
    queryFn: () => getAgentKnowledge(agentSlug),
    staleTime: 60_000,
  });

  const pages = query.data?.pages ?? [];
  // The selection is scoped to the agent it was made in, so switching agents
