...
import type { AgentKnowledgePage } from "../../api/agentKnowledge";
import { AgentKnowledgePanel } from "./AgentKnowledgePanel";

// The panel reads and writes through the real api/agentKnowledge client, so
...
function page(over: Partial<AgentKnowledgePage> = {}): AgentKnowledgePage {
  return {
...
describe("AgentKnowledgePanel", () => {
  ...
  it("cites the note each page came from", async () => {
    ...
    const { findByText } = wrap(<AgentKnowledgePanel agentSlug="dwight" />);
  ...
  it("promotes with the content hash it displayed, so the human publishes what they read", async () => {
    ...
    const { findByRole } = wrap(<AgentKnowledgePanel agentSlug="dwight" />);
    fireEvent.click(
  ...
  it("surfaces a rejected promotion instead of pretending it landed", async () => {
    ...
    const { findByRole, findByText } = wrap(
      <AgentKnowledgePanel agentSlug="dwight" />,
    );
    fireEvent.click(
      await findByRole("button", { name: /promote to team wiki/i }),
    );
  ...
  // the agent asked, not against the note as it stands now. The panel fetches
  ...
  it("reviews a pending promotion against the snapshot the agent asked with", async () => {
  ...
  it("shows the snapshot, not the live note, when the agent rewrote it after asking", async () => {
  ...
  it("renders agent-authored prose as text, never as markup", async () => {
    get.mockResolvedValue({
      agent: "dwight",
      pages: [
        page({
