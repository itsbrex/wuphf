...
const baseBot: OfficeMember = {
  slug: "planner",
  name: "Planner",
  role: "Plans tasks and coordinates bots",
  status: "active",
...
describe("<AgentProfilePanel>", () => {
  ...
  it("renders bot name, status badge, and close button", async () => {
    const onClose = vi.fn();
    render(wrap(<BotProfilePanel agent={baseBot} onClose={onClose} />));

    expect(screen.getByText("Planner")).toBeInTheDocument();
    expect(screen.getByLabelText("Close bot profile")).toBeInTheDocument();
    // Status badge should say "active"
    expect(screen.getByText("active")).toBeInTheDocument();
  });

  it("renders bot role description", async () => {
    render(wrap(<BotProfilePanel agent={baseBot} onClose={vi.fn()} />));
    expect(
      screen.getByText("Plans tasks and coordinates bots"),
    ).toBeInTheDocument();
  });
  ...
  it("calls onClose when close button is clicked", async () => {
    ...
    await user.click(screen.getByLabelText("Close bot profile"));
  ...
  it("treats malformed list responses as empty lists", async () => {
    ...
    listBotLogTasksMock.mockResolvedValue({ tasks: { broken: true } });
  ...
  it("filters malformed entries from list responses", async () => {
    ...
    listBotLogTasksMock.mockResolvedValue({ tasks: [null] });
  ...
  it("renders skills owned by this bot", async () => {
    ...
    // Skill owned by a different bot should NOT appear
  ...
  it("renders recent runs when log tasks are available", async () => {
    // The API is called with botSlug so the server filters; the mock
    // returns only the current bot's runs, matching real server behavior.
    listBotLogTasksMock.mockResolvedValue({
      tasks: [
        {
          taskId: "task-abc-123",
          agentSlug: "planner",
    ...
    // Verify the API was called with the bot-scoped params
    expect(listBotLogTasksMock).toHaveBeenCalledWith({
      limit: 8,
      agentSlug: "planner",
    });
  ...
  it("renders recent tasks for the bot", async () => {
  ...
  it("renders permissions section with correct role for non-lead bot", async () => {
    render(wrap(<BotProfilePanel agent={baseBot} onClose={vi.fn()} />));
  ...
  it("renders permissions section with correct role for built-in (lead) bot", async () => {
    ...
    render(wrap(<BotProfilePanel agent={leadBot} onClose={vi.fn()} />));
  ...
  it("handles missing optional fields on bot gracefully", async () => {
    const minimalBot: OfficeMember = {
      slug: "bare",
      name: "Bare",
      role: "",
      status: undefined,
    };
    render(wrap(<BotProfilePanel agent={minimalBot} onClose={vi.fn()} />));
    ...
    expect(
      screen.queryByText("Plans tasks and coordinates bots"),
