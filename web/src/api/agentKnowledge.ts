/**
 * Agent knowledge API client.
 *
 * Knowledge is what a given AGENT knows: the pages in that agent's own
 * notebook, read straight off the wiki repo. It is private to that agent until
 * a human promotes it into the shared team wiki.
 *
 * Two promotion paths, one gate:
 *  - `promoteAgentKnowledge`  a human promoting a page they are reading.
 *  - `requestKnowledgePromotion`  an agent asking a human to approve one.
 *
...
/**
 * One page an agent knows. The shape is the broker's knowledge page; the
 * agent-scoped fields (`agent`, `sourcePath`, `promotion`) are what make it
...
/** Read everything one agent knows. An agent with no notes returns no pages. */
...
export interface PromoteAgentKnowledgeInput {
  agentSlug: string;
  sourcePath: string;
...
export interface PromoteAgentKnowledgeResult {
  path: string;
  commitSha: string;
}

/** Promote a page into the shared wiki. Human-only; the broker enforces that. */
export async function promoteAgentKnowledge(
  input: PromoteAgentKnowledgeInput,
): Promise<PromoteAgentKnowledgeResult> {
 ...
 * agent asked, NOT a re-read — editing the note afterwards does not change it.
 */
...
/**
 * Raise the approval card an agent uses to ask for a promotion. Returns the
